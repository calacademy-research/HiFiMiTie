...
fasta=$2
[ -z "$fasta" ] && fasta=mito_rec_candidates.fasta
[ ! -s "$fasta" ] && fasta=../mito_rec_candidates.fasta
[ ! -s "$fasta" ] && msg.sh "add_goosehairpin_to_mitfi: can not find mito_rec_candidates.mitfi" && exit 2
...
awk '
   ...
   function look_for_gh(recid, trna_start_pos) {
      for(g = 1; g <= num_gh[recid]; g++) {
         ...
         if (gh_start_pos > 0 && gh_start_pos < trna_start_pos) { # insert the goosehaipin info before this trna info
            pos = gh_start_pos; end_pos = pos + length(gh_seq)-1
            printf("%s\t%d\t%d\t.\t.\t.\t%s\tgoose_hairpin\t+\n", recid, pos, end_pos, gh_seq)

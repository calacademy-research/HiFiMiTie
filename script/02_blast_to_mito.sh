...
[ ! -d $out_dir ] && mkdir $out_dir && msglog_module "created $(basename $out_dir) to hold the blast output tsv files"
...
function do_fofn_blast {
   ...
   # replace above with reading it from the settings. gettaxid called from 01_init.sh stores it there

   toptax=$(get_setting "taxid")
   ...
   eval_cutoff=1e-10
   set_blast_extra_args  # handles multi threading differently in version 2.12 and greater, should be faster when using taxidlist

   set_mitodb  # defined in shared.sh
   db=$mitodb_path
   ...
   while IFS= read -r file; do
      ...
      out=${out_base}.tsv
      [ -f ${out_base}.err ] && rm ${out_base}.err
